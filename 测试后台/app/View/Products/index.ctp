...
<!-- <ul class="edit_title mar_left_right clearfix sub_ContainerHeight">
    ...
    <li><a href="<?php echo $this->Html->url('/ExchangeLogs/index');?>">商品兑换</a></li> </ul> -->
    ...
    <div>
      <div class="select m_right ver_top m_right" style="float: left;">
        <ul style="display:none;" class="choiceStatus">
            ...
            <li value="4" >已撤销</li>
            <li value="5" >草稿箱</li>
            <!-- <li value="6" >置顶banner</li> -->
        </ul>
        <input type="hidden" name="sort" value="">
        <span style="display:none;">
            <input class="search-item"  type="hidden" name="status" value="<?php echo $this->request->query['status'];?>">
        </span>
        <a href="javascript:void(0);"  name="<?php echo $this->request->query['status'];?>">
            <?php if($this->request->query['status']==6){
               echo '置顶banner';
            }else{
               echo $ststusName[$this->request->query['status']]; 
            } ?></a>
      </div>
        <div class="select m_right ver_top m_right" style="float: left;">
            <ul style="display:none;" class="choiceStatus1" id="issort">
                <li value="2" >不限</li>
                <li value="1" >是</li>
                <li value="0" >否</li>
            </ul>
            <input type="hidden" name="sort" value="">
            <span style="display:none;">
                <input class="search-item"  type="hidden" name="sort" value="<?php echo $this->request->query['sort'];?>">
            </span>
           <a href="javascript:void(0);"  name="<?php echo $this->request->query['sort'];?>">
               <?php 
               if(!isset($this->request->query['sort'])||$this->request->query['sort']==''){
                      echo "<input placeholder='是否置顶banner'/>";
               }else{
                      if($this->request->query['sort']==1){
                      echo "是";
                      }elseif($this->request->query['sort']==0){
                              echo "否";
        ...
        <div class=" m_right ver_top m_right" style="float: left;">
            <input  class="Wdate search-item"  placeholder="&nbsp;&nbsp;开始时间" type="text" name="start_time" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:140px;height: 26px;" value="<?php echo $this->request->query['start_time'];?>">
        </div>
        <div style="padding-top:6px;padding-right:6px;float:left;">至</div>
        <div class="m_right" style="float: left;">
          <input  class="Wdate search-item" placeholder=" &nbsp;&nbsp;结束时间" type="text" name="end_time" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:140px;height: 26px;"  value="<?php echo $this->request->query['end_time'];?>">
        </div>
        <div class="search_box ver_top" style="float: left;">
            <input   type="text" name="activityName" class="search-item"  placeholder=" &nbsp;&nbsp;商品名称"  value="<?php echo $this->request->query['activityName'];?>" >
        ...
        <table class="table_list">
            ...
            <tr>
                <th>
                    ...
                    <label  onClick="selectAll()" for="all_check"></label>
                    ID
                </th>
                <th>
                    商品名称
            ...
            <tbody>
            <?php  foreach($activties as $k=> $activty){if($activty['Product']['status']==$this->request->query['status']||$activty['Product']['status']==1){?>
            <tr>
                <td>
                    <input  class="goods_check" type="checkbox"  id="<?php echo ($k+2);?>" data="<?php echo $activty['Product']["id"]; ?>"/>
                    <label onClick="selectList()" for="<?php echo ($k+2);?>" ></label>
                ...
                <td>
                    <?php echo $activty['Product']["sort"]?'是':'否'?>
                </td>
                 <td>
                    <?php echo $activty['Product']["rank"];?>
                </td>
                <td>
                ...
                <td>
                    <?php echo date("Y-m-d",$activty['Product']["start_time"]);?>
                </td>
                <td>
                    <?php echo date("Y-m-d",$activty['Product']["end_time"]);?>
                </td>
                <td>
...
<script>
    $(document).ready(function(){
        ...
        $("#issort >li").on('click',function(){
            $('input[name="sort"]').val($(this).attr('value'));
            // if($(this).text()=='不限'){
            //     $('input[name="sort"]').val("");
            // }
            search();
      ...
      var param="<?php echo $_SERVER["QUERY_STRING"];?>";
...
function selectAll(){
            if($("#all_check").is(':checked')){
                $(".goods_check").removeAttr("checked");
            }else{;
