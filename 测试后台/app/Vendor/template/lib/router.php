    ...
    private function _parse() {
        ...
        if (isset($pathinfo)) {
            if (isset(self::$_routers[$pathinfo])) {
                ...
                $patterns = array();
                if (is_array(self::$_routers))
                    foreach (self::$_routers as $pattern => $v)
                        $patterns[] = '#^' . $pattern . '$#';
                ...
                if ($str == $pathinfo) {
                    ...
                    if (!empty($args))
                        $str .= 'app=' . array_shift($args);
                    if (!empty($args))
                        $str .= '&controller=' . array_shift($args);
                    if (!empty($args))
                        $str .= '&action=' . array_shift($args);
                    if (!empty($args)) {
                        $str .= '&';
                        foreach ($args as $k => $v) {
                            $str .= $v . ($k % 2 == 0 ? '=' : '&');
