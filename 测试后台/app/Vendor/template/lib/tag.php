...
function tag_content($options) {
    ...
    $sql = 'SELECT DISTINCT ' . ($fields ? $fields : 'c.*') . ' FROM `#table_content` c ';
    $extrawhere = $where;
    $where = $join = $on = array();
    $where[] = "c.`status`=6";
    if (isset($contentid)) {
        ...
        if (!empty($tags)) {
            ...
            if (strpos($tags, ',') === false) {
                ...
                if ($tags) {
                    //    $sql .= ", `#table_content_tag` t";	//多个tag sql语句出现错误
                    $tagid = '';
                    foreach ($tags as $tag) {
                        $tagid .= $tag['tagid'] . ',';
                    }
        ...
        if (!empty($published)) {
            if (strpos($published, ',') === false) {
                ...
                if ($m[1])
                    $where[] = where_mintime('c.`published`', $m[1]);
                if ($m[2])
                    $where[] = where_maxtime('c.`published`', $m[2]);
            }
        }
    }
    if (!empty($extrawhere)) {
    ...
    if ($join)
        $sql .= " LEFT JOIN(" . implode(',', $join) . ") ON(" . implode(' AND ', $on) . ") ";
    $sql .= ' WHERE ' . implode(' AND ', $where);

    if (!empty($orderby)) {
        if (strpos($orderby, ',') !== false)
            $orderby = str_replace(',', ',c.', $orderby);
        $orderby = 'c.' . $orderby;
    }
    else {
...
function tag_db($options) {
    ...
    if ($data) {
        ...
        if (isset($charset) && $db->options['charset'] != $charset)
            $data = str_charset($db->options['charset'], $charset, $data);
        if ($page)
