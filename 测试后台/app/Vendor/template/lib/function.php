...
function id_format($id) {
    if (is_numeric($id))
        return $id;
    if (is_array($id))
        return array_filter($id, 'is_numeric');
    if (strpos($id, ',') !== false)
...
function str_cutword($string, $length = 80, $charset = "utf-8", $etc = '...') {
    ...
    if (function_exists("mb_substr"))
        return mb_substr($string, $start, $length, $charset) . $etc;
...
function thumb($img, $width, $height, $is_abs = 1, $default = null) {
    if (empty($img))
        return is_null($default) ? BASE_URL . 'images/nopic.gif' : $default;
    if (!extension_loaded('gd'))
        return $img;
    if (preg_match("/^(" . preg_quote(UPLOAD_URL, '/') . "|" . preg_quote(UPLOAD_PATH, '/') . ")(.*)$/", $img, $matches))
        $img = $matches[2];
    if (strpos($img, '://') || !file_exists(UPLOAD_PATH . $img))
        return $img;
...
function http_build_url($data, $mode = false) {
    ...
    if (isset($data['path']))
        $url .= $data['path'];
    if (isset($data['query']))
        $url .= '?' . ($mode ? str_replace('&', '&amp;', $data['query']) : $data['query']);
