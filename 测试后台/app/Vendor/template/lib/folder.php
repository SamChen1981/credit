...
class folder extends object {
    ...
    static function create($structure, $mode = 0755, $force = false) {
        ...
        if (is_file($structure)) {
            if (!$force || !@unlink($structure)) {
                self::$_errormsgs[] = sprintf('%s is a file', $dir);
                return false;
            }
        }
    ...
    static function move($source, $target) {
        ...
        if (!@rmdir($source))
            throw new ct_exception("can not rmdir $source");
        return true;
    }
        ...
        if (is_null($mode)) {
            ...
            foreach ($items as $v) {
                ...
                if (is_dir($v)) {
                    ...
                    if (!@copy($v, $to)) {
                        self::$_errormsgs[] = sprintf('can not copy file %s to %s', $v, $to);
                        return false;
                    }
                }
            }
            ...
            foreach ($files as $file) {
                if (is_file($file)) {
                    ...
                    if (!copy($file, $newfile)) {
                        self::$_errormsgs[] = sprintf('can not copy file %s to %s', $file, $newfile);
                        return false;
                    }
                }
            }
    ...
    static function find($path, $pattern, $mode = 'name', $deep = false, &$array = array()) {
        ...
        if ($mode == 'name') {
            ...
            foreach ($items as $item) {
                if (is_file($item) && preg_grep($pattern, file_get_contents($item)))
                    $array[] = $item;
            }
        }
        elseif ($mode == 'filemtime') {
            ...
            foreach ($items as $item) {
                if (is_file($item) && @filemtime($item) >= $filemtime)
                    $array[] = $item;
            }
        }
        if ($deep) {
            foreach ($items as $item) {
                if (is_dir($item))
                    self::find($item, $pattern, $mode, $deep, $array);
            }
        }
        return $array;
    }
